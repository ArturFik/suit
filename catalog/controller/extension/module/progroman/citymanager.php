...
class ControllerExtensionModuleProgromanCityManager extends Controller {
    ...
    public function confirm($params) {
        ...
        if ($show_confirm) {
            $data = [
                ...
                'confirm_redirect' => $this->progroman_citymanager->setting('disable_autoredirect') // авторедирект отключен
                    ? $this->progroman_citymanager->getRedirectUrl($params['url']) : ''
            ];
    ...
    public function save() {
        $fias_id = $this->request->post['fias_id'] ?? 0;
        $url = $this->request->post['url'] ?? '/';
        $json['success'] = $fias_id && $this->progroman_citymanager->setFias($fias_id);
        if ($json['success']) {
            if ($redirect_url = $this->progroman_citymanager->getRedirectUrl($url)) {
    ...
    public function confirmShown($force = false) {
        ...
        if ((is_numeric($time) && !$this->progroman_citymanager->setting('popup_user_answer')) || $force) {
            $this->session->data[$this->progroman_citymanager->getSessionKey()]['shown_confirm'] = true;
    ...
    public function startup() {
        ...
        if ($this->isModuleEnabled()) {
            ...
            if ($this->progroman_citymanager->setting('use_geoip')) {
                // Для теста: Москва 94.25.169.110, Тамбов 193.34.14.221, Казань 217.66.24.13, Раменское 91.214.97.249
                CityManager::addDriver(new \progroman\CityManager\Driver\Db());
            }
