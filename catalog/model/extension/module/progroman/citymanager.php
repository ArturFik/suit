...
class ModelExtensionModuleProgromanCityManager extends Model {
    ...
    public function getMessages($fias_ids = []) {
        $fias_ids = array_filter(array_map('intval', $fias_ids), fn($a) => $a > 0);
        if ($fias_ids) {
    ...
    public function ip2fias($long_ip) {
        return $this->db->query("SELECT * FROM prmn_cm_ip2fias WHERE " . (int)$long_ip . " BETWEEN ip_from AND ip_to")->row;
    }
}