		...
		if ($data['affiliate']) {
			...
			*/
			if(!empty($data['newsletter'])) {
				...
				$newsletterinfo = $this->model_newsletter_subscriber->getSubscriberByEmail($data['email']);
				if($newsletterinfo) {
					$this->model_newsletter_subscriber->defaultupdateNewsletter($newsletterinfo);
				} else {
					$this->model_newsletter_subscriber->defaultaddNewsletter($data);
				}
			}
			/*
			* Ci Newsletter Ends
	...
	public function editCustomer($customer_id, $data) {
			...
			*/
			if(isset($data['newsletter'])) {
				...
				$update_data['status'] = (($data['newsletter']) ?  1 : 2);

				$newsletterinfo = $this->model_newsletter_subscriber->getSubscriberByEmail($update_data['email']);
				if($newsletterinfo) {
					$this->model_newsletter_subscriber->defaultupdateNewsletter($update_data);
				} else{
					$this->model_newsletter_subscriber->defaultaddNewsletter($update_data);
				}
			}
			/*
			* Ci Newsletter Ends
