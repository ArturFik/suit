...
class ControllerMailOrder extends Controller {
	...
	public function add($order_info, $order_status_id, $comment, $notify) {
		...
		$mail->smtp_timeout = $this->config->get('config_mail_smtp_timeout');
                    ...
                    if (strpos($order_info['payment_code'], 'artbeznal') !== false && $this->config->get('payment_'.$order_info['payment_code'].'_status') && $this->config->get('payment_'.$order_info['payment_code'].'_email_attach') && $this->config->get('payment_'.$order_info['payment_code'].'_on_status_id') == $order_status_id){
                        if (isset($artbeznal)) { 
                           $mail->addAttachment($artbeznal); 
                        }
                        else {
                            $arthtml = $this->load->controller('extension/payment/artbeznal/getForm', $order_info);
                            $artoutput = $this->load->controller('extension/payment/artbeznalclone/artbeznalpdf/atPDF', array('html' => $arthtml, 'order_id' => $order_info['order_id']));
                            $attachment = $artbeznal = DIR_UPLOAD . 'invoice-order_' . $order_info['order_id'] . '.pdf';
                            file_put_contents($artbeznal, $artoutput);
                            $mail->addAttachment($artbeznal);
                        }
                    } 
