...
class ControllerStartupSeoUrl extends Controller {
	public function index() {
      ...
      if ($this->config->get('mlseo_enabled')) {
        ...
        if (!empty($_SERVER['HTTP_HOST'])) {
          ...
          $uri_noparams = strtok($raw_uri, '?');
          $uri_params = parse_url($raw_url, PHP_URL_QUERY);
          ...
          if ($this->config->get('mlseo_redirect')) {
            ...
            $redirectQuery = $this->db->query("SELECT redirect, language_id FROM " . DB_PREFIX . "url_redirect WHERE
              ...
              OR (redirect LIKE 'product/product&product_id=%' AND query =  '" . $this->db->escape(urldecode(substr(strrchr($uri_noparams, '/'), 0))) . "')
              OR (redirect LIKE 'product/category&path=%' AND query =  '" . $this->db->escape(urldecode(substr(strrchr($uri_noparams, '/'), 0))) . "')")->rows;
            ...
            if (!empty($redirect['redirect'])) {
              ...
              if ((substr($redirect, 0, 1) != '/') && (substr($redirect, 0, 4) != 'http')) {
                if ($params = strstr($redirect, '&')) {
                  ...
                  $params = substr(str_replace('&amp;', '&', $params), 1);
                ...
                $redirectCurrent = str_replace('&amp;', '&', $this->url->link($route, $params));

                if ($redirectCurrent != $url) {
                  if ($lang) {
                  ...
                  $redirect = str_replace('&amp;', '&', $this->url->link($route, $params));
                } else {
      ...
      */
      ...
      if ($this->config->get('mlseo_url_absolute')) {
        ...
        if (strpos($fullUrl, '?')) {
          ...
          $searchWithWildcard = " OR redirect = '" .  $this->db->escape($fullUrl.'*') . "' ";
        }
      ...
      foreach ($parts as $part) {
        ...
        if ($this->config->get('mlseo_preload')) {
          $query = $this->getSeoUrl($part, true);
        } else {
        ...
        if ($this->config->get('mlseo_store_mode') && $this->config->get('mlseo_disable_other_store_links') && $this->request->get['route'] != 'journal3/blog') {
          ...
          if (isset($url[1]) && !in_array($url[0], array('route', 'product_id', 'category_id', 'information_id', 'manufacturer_id', 'blog_article_id'))) {
			...
			$this->request->get['blogpath'] .= '_' . $url[1];
					...
					if (empty($this->request->get['route']) && isset($query->row['query']) && !strpos($query->row['query'], '=') && $url[0] != 'route') if ($query->row['query'] && $url[0] != 'information_id' && $url[0] != 'manufacturer_id' && $url[0] 
  ...
  public function getSeoUrl($key, $get = false) {
    ...
    if ($get && $query = array_search($key, self::$seo_urls)) {
      $resUrl->num_rows = 1;
      $resUrl->row = array('query' => $query);
    } else if (isset(self::$seo_urls[$key])) {
      $resUrl->num_rows = 1;
