...
class ModelAccountCustomer extends Model {
	public function addCustomer($data) {
		if (isset($data['customer_group_id']) && is_array($this->config->get('config_customer_group_display')) && in_array($data['customer_group_id'], $this->config->get('config_customer_group_display'))) {
 		...
 		} elseif ($this->config->get('module_progroman_citymanager_status')) {
            $customer_group_id = $this->progroman_citymanager->getCustomerGroupForNewUser();
		} else {
		...
		*/
		if(!empty($data['newsletter'])) {
			...
			$newsletterinfo = $this->model_newsletter_cinewsletter->getNewsletterByEmail($data['email']);
			if($newsletterinfo) {
				$this->model_newsletter_cinewsletter->defaultupdateNewsletter($data);
			} else {
				$this->model_newsletter_cinewsletter->defaultaddNewsletter($data);
			}
		}
		/*
		* Ci Newsletter Ends
	...
	public function editNewsletter($newsletter) {
		...
		*/
		if(isset($newsletter)) {
			...
			$update_data = array();
			$update_data['name'] = $this->customer->getFirstName() .' '. $this->customer->getLastName();
			$update_data['email'] = $this->customer->getEmail();
			$update_data['account_register'] = 1;
			$update_data['status'] = (($newsletter) ?  1 : 2);

			$newsletterinfo = $this->model_newsletter_cinewsletter->getNewsletterByEmail($update_data['email']);
			if($newsletterinfo) {
				$this->model_newsletter_cinewsletter->defaultupdateNewsletter($update_data);
			} else{
				$this->model_newsletter_cinewsletter->defaultaddNewsletter($update_data);
			}
		}
		/*
		* Ci Newsletter Ends
