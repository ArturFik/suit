...
class ModelExtensionModuleProgromanCityManager extends Model {
    ...
    public function getData($key) {
        $row = $this->db->query("SELECT `value` FROM prmn_cm_data WHERE `key` = '" . $this->db->escape($key) . "'")->row;
    ...
    public function setData($key, $value) {
        ...
        $this->db->query("INSERT INTO prmn_cm_data (`key`, `value`) VALUES\n"
            . "('" . $this->db->escape($key) . "', '" . $this->db->escape($value) . "')\n"
            . "ON DUPLICATE KEY UPDATE `value` = VALUES(`value`)");
    }

    public function countIp2Fias() {
        return $this->db->query("SELECT EXISTS(SELECT 1 FROM prmn_cm_ip2fias) AS `exists`")->row['exists'];
    }

    public function clearIp2Fias() {
        return $this->db->query("TRUNCATE prmn_cm_ip2fias");
    }

    /**
     * Возвращает true, если установлена lite-версия базы страны (только города)
     * @param $fias_start
     * @param $fias_end
     * @return bool
     * @throws Exception
     */
    public function isLiteInstalled($fias_start, $fias_end) {
        return $this->db->query("SELECT COUNT(*) total FROM fias WHERE fias_id BETWEEN " . (int)$fias_start . " AND " . (int)$fias_end . " AND level > " . self::LEVEL_CITY)->row['total'] > 0;
    }
    ...
    private function installPrmnTables() {
        ...
        $this->db->query("
            CREATE TABLE `prmn_cm_ip2fias` (
              `ip_from` int(10) UNSIGNED NOT NULL,
              `ip_to` int(10) UNSIGNED NOT NULL,
              `fias_id` int(11) UNSIGNED NOT NULL,
              KEY `ip` (`ip_from`,`ip_to`)
            ) ENGINE=MyISAM DEFAULT CHARSET=utf8;");

        $this->db->query("
            CREATE TABLE `prmn_cm_data` (
    ...
    private function installDemoPrmnTables() {
        ...
        if (empty($this->db->query("SELECT COUNT(*) total FROM prmn_cm_data")->row['total'])) {
            $this->db->query("
                INSERT INTO `prmn_cm_data` (`key`, `value`) VALUES
                    ('citymanagerpro.countries', '[{\"name\":\"Россия\",\"iso\":\"ru\",\"fias_id\":1,\"fias_start\":2,\"fias_end\":299999,\"lite_exists\":true}]'), ('citymanagerpro.version_ip', 0), ('citymanagerpro.message', ''), ('citymanagerpro.updated_at', NULL)");
        }
    }
