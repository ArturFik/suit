...
public function updateProduct($data, $link, $setting, $dn_id, $pr_id=0, $browser){
	...
	//получаем списко используемых языков
	$langs = $this->getLang($setting);
	//переменная для запроса обновления атрибутов.
	$language_ids = implode(",", array_column($langs, 'language_id'));
	...
	if($pr_id > 0){
		...
		if($setting['r_price'] == 1){
			...
			$temp_price = $this->db->query("SELECT price FROM " . DB_PREFIX . "product WHERE product_id = ".(int)$pr_id." AND price > '".(float)$data['price']."'")->num_rows;
			...
			foreach($data['attr'] as $attr){
				...
				if($attr['id'] == 0){
					...
					if($check_attr->num_rows > 0){
						...
						$attr[1] = trim($attr[1]);
						$this->db->query("UPDATE " . DB_PREFIX . "product_attribute SET text='" . $this->db->escape($attr[1]) . "' " .
											"WHERE product_id=" . (int)$pr_id . " AND attribute_id=" . (int)$attr['id'] . " AND language_id IN (" . $language_ids . ")");
