...
use progroman\CityManager\DatabaseFile\Ip2Fias;
...
class ControllerExtensionModuleProgromanCityManager extends Controller {
    ...
    public function index() {
        ...
        $data['url_check_update'] = $this->makeModuleUrl('checkupdate');
        $data['url_remove_data_message'] = $this->makeModuleUrl('removedatamessage');

        $data['valid_license'] = $this->validLicense();
        $updated = $this->model_citymanager->getData('citymanagerpro.updated_at');
        $data['check_update'] = $data['valid_license'] && (time() - $updated > 60 * 60 * 24 * 30);

        $data['is_loaded_fias'] = count($this->model_citymanager->getInstalledCountries()) > 0;
        ...
        $data['message'] = nl2br($this->model_citymanager->getData('citymanagerpro.message'));
    ...
    public function general() {
        ...
        $data['use_default_city_values'] = [
            'nothing' => $this->language->get('text_not_determined'),
            'any_country' => $this->language->get('text_any_country'),
            'one_country' => $this->language->get('text_one_country'),
        ];

        $data['is_loaded_ip2fias'] = $this->model_citymanager->countIp2Fias();
    ...
    public function bases() {
        $data['base_ip'] = new Ip2Fias($this->language->all());
        $data['url_bases'] = $this->makeModuleUrl('bases');
        $data['download_files'] = [];
        foreach (Countries::getList() as $country) {
            $base_cities = new BaseCities($this->language->all());
            $base_cities->setCountry($country);
            $data['download_files'][] = $base_cities;
        }

        $data['is_loaded_fias'] = count($this->model_citymanager->getInstalledCountries()) > 0;
        $data['countries_not_found'] = $this->model_citymanager->getNoRelativeCountries();
        $data['zones_not_found'] = $this->model_citymanager->getNoRelativeZones();
    ...
    public function getSecretKey() {
        ...
        $mmc = new MMC(CityManager::getPublicKey(), $this->config->get('config_admin_language'), CityManager::MODULE_NAME, CityManager::VERSION);
        $key = $mmc->getSecretKey();
    ...
    public function checkUpdate() {
        ...
        if ($this->hasPermission()) {
            $updated = $this->model_citymanager->getData('citymanagerpro.updated_at');
            if (time() - $updated > 60 * 60 * 24 * 30) {
                $mmc = new MMC(CityManager::getPublicKey(), $this->config->get('config_admin_language'), CityManager::MODULE_NAME, CityManager::VERSION);
                $data = $mmc->checkUpdate();
                if (!$data) {
                    $json['error'] = $this->language->get('error_check_update') . ':<br>' . implode("<br>", $mmc->getErrors());
                    return $this->sendJson($json);
                }

                if (!empty($data['countries'])) {
                    $this->model_citymanager->setData('citymanagerpro.countries', $data['countries']);
                }

                $message = !empty($data['message']) ? $data['message'] . "\n" : '';

                if (!empty($data['latest_version_module']) && version_compare($data['latest_version_module'], CityManager::VERSION, '>')) {
                    $message .= $this->language->get('text_module_update_available') . "\n";
                }

                if (!empty($data['latest_version_ip'])) {
                    $this->model_citymanager->setData('citymanagerpro.latest_version_ip', $data['latest_version_ip']);
                    $version_ip = $this->model_citymanager->getData('citymanagerpro.version_ip');
                    if ($data['latest_version_ip'] > $version_ip) {
                        $message .= $this->language->get('text_ip_update_available') . "\n";
                    }
                }

                $this->model_citymanager->setData('citymanagerpro.updated_at', time());
                if ($message) {
                    $this->model_citymanager->setData('citymanagerpro.message', $message);
                    $json['message'] = nl2br($message);
                }
            }
    ...
    public function removeDataMessage() {
        if ($this->hasPermission()) {
            $this->model_citymanager->setData('citymanagerpro.message', '');
        }
    ...
    public function baseAction() {
        ...
        $action->setMMC(new MMC(CityManager::getPublicKey(), $this->config->get('config_admin_language'), CityManager::MODULE_NAME, CityManager::VERSION));
